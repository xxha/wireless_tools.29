...
#define MAX_WIFI_SCAN_ITEM      20
static int             vespec_ap_last = 0;
static int             vespec_ap_number = 0;
static IWSWiFiScanItem vespec_ap_scanitem;

static int      flag_out_verbose = 0;
...
const char* const veex_iw_mode[] = { "auto",
					"ad-hoc",
					"managed",
					"master",
					"mepeater",
...
static inline void 
iw_extract_ie_wpa(char *enctype,
...
static inline void
iw_extract_gen_ie (char *enctype,
		unsigned char *	buffer, int	buflen)
{
  ...
  while(offset <= (buflen - 2))
	...
	case 0xdd:	/* WPA1 (and other) */
	case 0x30:	/* WPA2 */
	  iw_extract_ie_wpa(enctype, buffer + offset, buffer[offset+1] + 2);
	  break;
	default:
      flag_unknown = 1;
	}
 ...
 */
static inline void
print_scanning_token_this()
{
    int  enc_len = 0, enc_flag = 0;

    enc_flag = vespec_ap_scanitem.encryption;
    enc_len  = strlen(vespec_ap_scanitem.enctype);

    fprintf(stdout, "wifiscan: stat> CellNo=%d, Address=%s, ESSID=%s, "
                        "Mode=%s, Frequency=%s, Channel=%d, Quality=%s, Signal=%s, Noise=%s, "
                        "EncKey=%d, EncType=%s\n", 
                     vespec_ap_number,
                     vespec_ap_scanitem.bssid,
                     vespec_ap_scanitem.essid,
                     vespec_ap_scanitem.mode, vespec_ap_scanitem.str_frequency, vespec_ap_scanitem.channel,
                     vespec_ap_scanitem.str_quality,
                     vespec_ap_scanitem.str_signal,
                     vespec_ap_scanitem.str_noise,
                     vespec_ap_scanitem.encryption,
					 ((enc_len == 0) ? ((enc_flag == 0) ? "Off" : "WEP") : vespec_ap_scanitem.enctype));
	fflush(stdout);

    vespec_ap_last = vespec_ap_number;
}

static void
iw_print_stats_this(const iwqual *	qual,
	       const iwrange *	range,
	       int		has_range)
{
  bzero(vespec_ap_scanitem.str_quality, sizeof(vespec_ap_scanitem.str_quality));
  bzero(vespec_ap_scanitem.str_signal, sizeof(vespec_ap_scanitem.str_signal));
  bzero(vespec_ap_scanitem.str_noise, sizeof(vespec_ap_scanitem.str_noise));
  ...
  if(has_range && ((qual->level != 0)
	...
	{
	  snprintf(vespec_ap_scanitem.str_quality, sizeof(vespec_ap_scanitem.str_quality)-1,
			 "%d/%d",
	  ...
	  if(!(qual->updated & IW_QUAL_LEVEL_INVALID))
	    {
	      ...
	      snprintf(vespec_ap_scanitem.str_signal, sizeof(vespec_ap_scanitem.str_signal)-1,
			     "%g dBm",
			     rcpilevel);
	    }
	  ...
	  if(!(qual->updated & IW_QUAL_NOISE_INVALID))
	    {
	      ...
	      snprintf(vespec_ap_scanitem.str_noise, sizeof(vespec_ap_scanitem.str_noise)-1,
	             "%g dBm",
			     rcpinoise);
	    }
	}
  ...
  if (vespec_ap_scanitem.str_quality[0] == '\0')
    vespec_ap_scanitem.str_quality[0] = ' ';
  if (vespec_ap_scanitem.str_signal[0] == '\0')
    vespec_ap_scanitem.str_signal[0] = ' ';
  if (vespec_ap_scanitem.str_noise[0] == '\0')
    vespec_ap_scanitem.str_noise[0] = ' ';
}
		     ...
		     int		has_range)
{
  ...
  if (flag_out_verbose)
  {
    fprintf(stdout, "debug: decode event(cmd=0x%4X) ...\n",
            event->cmd);
    fflush(stdout);
  }
  ...
  switch(event->cmd)
    {
    case SIOCGIWAP:
      if (vespec_ap_number > 0)
      {
          print_scanning_token_this();
      }
      vespec_ap_number += 1;
      bzero(&vespec_ap_scanitem, sizeof(vespec_ap_scanitem));
	  strncpy(vespec_ap_scanitem.bssid,
                    iw_saether_ntop(&event->u.ap_addr, buffer),
                    sizeof(vespec_ap_scanitem.bssid)-1);
      break;
    case SIOCGIWFREQ:
      {
	...
	iw_print_freq_value(vbuf, sizeof(vbuf), freq);
	    ...
	    vespec_ap_scanitem.channel = channel;
	    sprintf(vespec_ap_scanitem.str_frequency, "%.3f GHz", freq/1000000000);
      }
      break;
    case SIOCGIWMODE:
	...
	event->u.mode = IW_NUM_OPER_MODE;
	  /* 0: Let the driver decides */
	  /* 1: Single cell network */
	  /* 2: Multi cell network, roaming, ... */
	  /* 3: Synchronisation master or Access Point */
	  /* 4: Wireless Repeater (forwarder) */
	  /* 5: Secondary master/repeater (backup) */
	  strncpy(vespec_ap_scanitem.mode, veex_iw_mode[event->u.mode],
                    sizeof(vespec_ap_scanitem.mode)-1);
      break;
    case SIOCGIWESSID:
      {
	...
	if(event->u.essid.flags)
	  {
			strncpy(vespec_ap_scanitem.essid, essid,
                    sizeof(vespec_ap_scanitem.essid)-1);
	  }
	else
	  {
			strncpy(vespec_ap_scanitem.essid, "off/any/hidden",
                    sizeof(vespec_ap_scanitem.essid)-1);
	  }
      }
      break;
	...
	if(event->u.data.flags & IW_ENCODE_DISABLED)
	      vespec_ap_scanitem.encryption = 0;
	else
	      vespec_ap_scanitem.encryption = 1;
      }
      break;
#if 0
    case SIOCGIWRATE:
	  {
		  double	rate = event->u.bitrate.value;
		  char		scale;
		  int		divisor;
		  ...
		  else
		  {
  		  ...
  		  snprintf(buffer, sizeof(buffer), "%g%cb/s", rate / divisor, scale);
      	  printf("BitRates=%s\n", buffer);
	  }
      break;
#endif
    case IWEVQUAL:
	  ...
	  bzero(vespec_ap_scanitem.str_signal, sizeof(vespec_ap_scanitem.str_signal));
	  bzero(vespec_ap_scanitem.str_noise, sizeof(vespec_ap_scanitem.str_noise));
      iw_print_stats_this(&event->u.qual, iw_range, has_range);
      break;
#ifndef WE_ESSENTIAL
    case IWEVGENIE:
      /* Informations Elements are complex, let's do only some of them */
      //iw_print_gen_ie(event->u.data.pointer, event->u.data.length);
      iw_extract_gen_ie(vespec_ap_scanitem.enctype, event->u.data.pointer, event->u.data.length);
      break;
#endif	/* WE_ESSENTIAL */
    default:
      if (flag_out_verbose)
      {
        fprintf(stdout, "debug: unregonized event(cmd=0x%4X).\n",
            event->cmd);
        fflush(stdout);
      }
   }	/* switch(event->cmd) */
  ...
  if (flag_out_verbose)
  {
    fprintf(stdout, "debug: decode event(cmd=0x%4X) done!\n",
            event->cmd);
    fflush(stdout);
  }
}
		    ...
		    int		count)		/* Args count */
{
  ...
  /* Init VeeX debug scanning buffer */
  bzero(&vespec_ap_scanitem, sizeof(vespec_ap_scanitem));
	...
	if(!strncmp(args[0], "timeout", 7))
	  {
	    ...
	    args++;
	    count--;
        timeout = atoi(args[0]) * 1000000; /* default 15s */
	  }
      else
	/* Scan with specific verbose */
	if(!strncmp(args[0], "verbose", 7))
	  {
        flag_out_verbose = 1;
	  }
	else
  ...
  if(wrq.u.data.length)
    {
      ...
      /* Debugging code. In theory useless, because it's debugged ;-) */
      int	i;
      fprintf(stdout, "Scan result %d [%02X", wrq.u.data.length, buffer[0]);
	...
	fprintf(stdout, ":%02X", buffer[i]);
      ...
      if (flag_out_verbose)
      {
        fprintf(stdout, "debug: scan aompleted, init event stream ...!\n");
        fflush(stdout);
      }
      iw_init_event_stream(&stream, (char *) buffer, wrq.u.data.length);
      if (flag_out_verbose)
      {
        fprintf(stdout, "debug: init event stream done!\n");
        fflush(stdout);
      }
      do
	{
      ...
      if (flag_out_verbose)
      {
        fprintf(stdout, "debug: extract event from stream ...\n");
        fflush(stdout);
      }
      ...
      if (flag_out_verbose)
      {
        fprintf(stdout, "debug: extract event from stream done(ret=%d)\n",
                    ret);
        fflush(stdout);
      }
	  if(ret > 0)
	  {
	    print_scanning_token(&stream, &iwe,
				 &range, has_range);
	  }
	}
      while(ret > 0);
      if (vespec_ap_number > vespec_ap_last)
      {
          print_scanning_token_this();
      }
      ...
      if (flag_out_verbose)
      {
        fprintf(stdout, "debug: event stream extract done!\n");
        fflush(stdout);
      }

    fprintf(stdout, "wifiscan: over\n");
	fflush(stdout);
    }
  else
    {
      fprintf(stdout, "wifiscan: NoResults\n");
	  fflush(stdout);
    }
...
static const struct iwlist_entry iwlist_cmds[] = {
  { "scanning",		print_scanning_info,	-1, "[essid NNN] [verbose] [timeout T]" },
